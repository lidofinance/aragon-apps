...
contract Voting is IForwarder, AragonApp {
    ...
    bytes32 public constant UNSAFELY_MODIFY_VOTE_TIME_ROLE =
    ...
    string private constant ERROR_CHANGE_VOTE_TIME =
        "VOTING_VOTE_TIME_TOO_SMALL";
    string private constant ERROR_CHANGE_OBJECTION_TIME =
        "VOTING_OBJ_TIME_TOO_BIG";
    string private constant ERROR_INIT_OBJ_TIME_TOO_BIG =
        "VOTING_INIT_OBJ_TIME_TOO_BIG";
    string private constant ERROR_CAN_NOT_VOTE_FOR = "VOTING_CAN_NOT_VOTE_FOR";
    string private constant ERROR_ZERO_ADDRESS_PASSED =
        "VOTING_ZERO_ADDRESS_PASSED";
    ...
    string private constant ERROR_DELEGATE_SAME_AS_PREV =
        "VOTING_DELEGATE_SAME_AS_PREV";
    ...
    string private constant ERROR_MAX_DELEGATED_VOTERS_REACHED =
        "VOTING_MAX_DELEGATED_VOTERS_REACHED";
    ...
    function unsafelyChangeVoteTime(
        uint64 _voteTime
    ) external auth(UNSAFELY_MODIFY_VOTE_TIME_ROLE) {
    ...
    function vote(
        ...
        require(
            _hasVotingPower(votes[_voteId], msg.sender),
            ERROR_NO_VOTING_POWER
        );
    ...
    function setDelegate(address _delegate) public {
        ...
        uint256 votingPower = token.balanceOfAt(
            msgSender,
            getBlockNumber64() - 1
        );
    ...
    function voteForMultiple(
        ...
        for (uint256 i = 0; i < _voters.length; ++i) {
            ...
            require(
                _canVoteFor(vote_, msgSender, voter),
                ERROR_CAN_NOT_VOTE_FOR
            );
    ...
    function canExecute(
        uint256 _voteId
    ) public view voteExists(_voteId) returns (bool) {
    ...
    function canVote(
        ...
        Vote storage vote_ = votes[_voteId];
        return
            _canParticipateInVote(_voteId, false) &&
            _hasVotingPower(vote_, _voter);
    }
    ...
    function getVotePhase(
        uint256 _voteId
    ) external view voteExists(_voteId) returns (VotePhase) {
    ...
    function getVoterState(
        uint256 _voteId,
        address _voter
    ) public view voteExists(_voteId) returns (VoterState) {
     ...
     */
    function getDelegatedVotersCount(
        address _delegate
    ...
    */
    function _vote(
        ...
        if (_supports) {
            ...
            vote_.voters[_voter] = _isDelegate
    ...
    function _addDelegatedAddressFor(
        ...
        delegates[_voter] = Delegate(
            _delegate,
            uint96(delegatedVoters[_delegate].addresses.length.sub(1))
        );
    }
     ...
     * @return the array of voting power of delegated voters
     */
    function _getDelegatedVotersAt(
        address _delegate,
        uint256 _offset,
        uint256 _limit,
        ...
        returns (address[] memory votersList, uint256[] memory votingPowerList)
    {
        ...
        uint256 returnCount = _offset.add(_limit) > length
            ? length.sub(_offset)
            : _limit;
    ...
    function _canExecute(uint256 _voteId) internal view returns (bool) {
        ...
        // Has min quorum?
        if (
            !_isValuePct(voteYea, vote_.votingPower, vote_.minAcceptQuorumPct)
        ) {
            return false;
        }
     ...
     */
    function _canParticipateInVote(
        uint256 _voteId,
        bool _supports
     ...
     */
    function _getVotePhase(
        Vote storage vote_
    ) internal view returns (VotePhase) {
