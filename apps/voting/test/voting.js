...
contract(
  ...
  ([
    ...
    before("load roles", async () => {
      ...
      UNSAFELY_MODIFY_VOTE_TIME_ROLE =
        await votingBase.UNSAFELY_MODIFY_VOTE_TIME_ROLE();
    });
    ...
    for (const decimals of [0, 2, 18, 26]) {
      context(`normal token supply, ${decimals} decimals`, () => {
        ...
        it("check castVote do nothing (deprecated)", async () => {
          ...
          assert.equal(
            await voting.getVoterState(voteId, holder51),
            VOTER_STATE.ABSENT,
            "holder51 should not have voted"
          );
        });
        ...
        it("check executesIfDecided do nothing (deprecated)", async () => {
          ...
          assert.equal(
            ...
            false,
            "should be in the unexecuted state"
          );
          await voting.mockIncreaseTime(votingDuration + 1);
          await voting.executeVote(voteId);
          assert.equal(
            await voting.canExecute(voteId),
            false,
            "should be in the executed state"
          );
        ...
        context("creating vote", () => {
          ...
          beforeEach(async () => {
            ...
            ](script, "metadata", false, false, { from: holder51 });
          ...
          it("changing min quorum doesnt affect vote min quorum", async () => {
            ...
            const tx = await voting.vote(voteId, true, true, {
              from: holder29,
            });
            assertEvent(tx, "CastVote", {
              expectedArgs: { voteId: voteId, voter: holder29, supports: true },
            });
          ...
          it("holder can vote", async () => {
            const tx = await voting.vote(voteId, false, true, {
              from: holder29,
            });
            assertEvent(tx, "CastVote", {
              expectedArgs: {
          ...
          it("holder can object", async () => {
            ...
            await assertRevert(
              voting.vote(voteId, true, false, { from: holder29 }),
            ...
            let tx = await voting.vote(voteId, false, false, {
              from: holder29,
            });
            assertEvent(tx, "CastVote", {
              expectedArgs: {
            ...
            assertEvent(tx, "CastObjection", {
              expectedArgs: { voteId: voteId, voter: holder29 },
            });
            assertAmountOfEvents(tx, "CastVote", { expectedAmount: 1 });
          ...
          it("getVotePhase works", async () => {
            ...
            const MAIN_PHASE = 0;
            assert.equal(
              await voting.getVotePhase(voteId),
              MAIN_PHASE,
              "should be main phase"
            );
            assert.equal(
              await extractPhaseFromGetVote(voteId),
              MAIN_PHASE,
              "should be main phase"
            );
          ...
          it("vote isnot executed automatically if decided", async () => {
            ...
            assert.equal(
              await executionTarget.counter(),
              0,
              "should not have executed result"
            );
          });
      ...
      context("voting for", () => {
        ...
        it("delegate can vote for voter", async () => {
          const tx = await voting.voteFor(voteId, false, holder29, {
            from: delegate1,
          });
          assertEvent(tx, "CastVote", {
            expectedArgs: { voteId: voteId, voter: holder29, supports: false },
          });
          ...
          assert.equal(
            voterState,
            VOTER_STATE.DELEGATE_NAY,
            "holder29 should have delegate nay voter status"
          );
        });

        it("delegate can vote for both voters", async () => {
          const tx = await voting.voteForMultiple(
            voteId,
            false,
            [holder29, holder51],
            { from: delegate1 }
          ...
          assertEvent(tx, "CastVote", {
            index: 1,
            expectedArgs: { voteId: voteId, voter: holder51, supports: false },
          });
          assertEvent(tx, "CastVoteAsDelegate", {
            expectedArgs: {
              voteId: voteId,
          ...
          assert.equal(
            voterState51,
            VOTER_STATE.DELEGATE_NAY,
            "holder51 should have delegate nay voter status"
          );
        });
        ...
        it(`delegate can't vote for both voters if one has previously voted`, async () => {
          await voting.vote(voteId, false, true, { from: holder29 });
          await assertRevert(
            voting.voteForMultiple(voteId, false, [holder29, holder51], {
              from: delegate1,
            }),
        ...
        it(`voter can overwrite delegate's vote`, async () => {
          ...
          assertBn(
            state[7],
            bigExp(0, decimals),
            "nay vote should have been reset"
          );
          ...
          assert.equal(
            voterState29,
            VOTER_STATE.YEA,
            "holder29 should have yea voter status"
          );
        });
    ...
    context("token supply = 3", () => {
      ...
      it("new vote cannot be executed before holder2 voting and time pass", async () => {
        ...
        assert.isFalse(
          (await voting.getVote(voteId)).executed,
          "vote should not have been executed"
        );
    ...
    context("unsafe time change", () => {
      ...
      it("reverts on non-authorized vote time change request", async () => {
        ...
        for (ind = 0; ind < entities.length; ++ind) {
          await assertRevert(
            voting.unsafelyChangeVoteTime(500, { from: entities[ind] }),
            "APP_AUTH_FAILED"
          );
        }
      });
      ...
      it("simple change vote time", async () => {
        ...
        assert.equal(
          ...
          smallest,
          "should have changed acceptance time"
        );
      ...
      it("reverts on non-authorized obj time change request", async () => {
        ...
        for (ind = 0; ind < entities.length; ++ind) {
          await assertRevert(
            voting.unsafelyChangeObjectionPhaseTime(100, {
              from: entities[ind],
            }),
            "APP_AUTH_FAILED"
          );
      ...
      it("simple change obj time", async () => {
        ...
        assert.equal(
          await voting.objectionPhaseTime(),
          increasingTime,
          "should have changed acceptance time"
        );
      ...
      it("reverts if voteTime < objectionTime", async () => {
        await assertRevert(
          voting.unsafelyChangeObjectionPhaseTime(votingDuration + 1),
          ERRORS.VOTING_OBJ_TIME_TOO_BIG
        );

        await voting.unsafelyChangeObjectionPhaseTime(votingDuration - 1);

        await assertRevert(
          voting.unsafelyChangeVoteTime(votingDuration - 1),
          ERRORS.VOTING_VOTE_TIME_TOO_SMALL
        );
        await assertRevert(
          voting.unsafelyChangeVoteTime(0),
          ERRORS.VOTING_VOTE_TIME_TOO_SMALL
        );
      });
      ...
      it("re-open finished vote through changing of voting time", async () => {
        ...
        assert.isTrue(
          voteState[0],
          "vote should be open after increasing of voting time"
        );
        assert.isFalse(voteState[1], "vote should not be executed");
      });
      ...
      it("close vote through changing of voting time", async () => {
        ...
        assert.isFalse(
          voteState[0],
          "vote should be closed after time decreasing"
        );
        assert.isFalse(voteState[1], "vote should not be executed");
      });
      ...
      it("changing time does not affect executed votes", async () => {
        ...
        assert.isFalse(
          voteState[0],
          "vote should be closed after time increasing"
        );
        assert.isTrue(voteState[1], "vite should be executed");
      });
    });
    ...
    context("voting delegate", () => {
      ...
      it("voter can set delegate", async () => {
        ...
        assert.equal(
          delegate,
          delegate1,
          "holder29 should have delegate1 as a delegate"
        );

        const delegatedVoters = (
          await voting.getDelegatedVoters(delegate1, 0, 1)
        )[0];
        assertArraysEqualAsSets(
          delegatedVoters,
          [holder29],
          "delegate1 should be a delegate of holder29"
        );
      });
      ...
      it("voter can remove delegate", async () => {
        ...
        assertArraysEqualAsSets(
          delegatedVoters,
          [],
          "delegate1 should not be a delegate of anyone"
        );
      });
      ...
      it("voters can remove delegate", async () => {
        ...
        assertArraysEqualAsSets(
          delegatedVoters,
          [holder20],
          "delegate1 have only holder20 as a delegated voter"
        );
      });
      ...
      it("voter can change delegate", async () => {
        ...
        const delegatedVotersDelegate1 = (
          await voting.getDelegatedVoters(delegate1, 0, 1)
        )[0];
        assertArraysEqualAsSets(
          delegatedVotersDelegate1,
          [],
          "delegate1 should not be a delegate of anyone"
        );
        const delegatedVotersDelegate2 = (
          await voting.getDelegatedVoters(delegate2, 0, 2)
        )[0];
        assertArraysEqualAsSets(
          delegatedVotersDelegate2,
          [holder29, holder51],
          "delegate2 should be a delegate of holder29 and holder51"
        );
      });
      ...
      it("delegate can manage several voters", async () => {
        ...
        const delegatedVoters = (
          await voting.getDelegatedVoters(delegate1, 0, 2)
        )[0];
        assertArraysEqualAsSets(
          delegatedVoters,
          [holder29, holder51],
          "delegate1 should be a delegate of holder29 and holder51"
        );
      });
    });
  }
